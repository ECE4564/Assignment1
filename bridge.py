...
print(time() + "Created socket at 0.0.0.0  on  port {0}".format(bridge_port))
...
while True:
    ...
    print(time() + "Decrypt :  Key : ", data[0], " |  Plaintext ", plaintext)

    watson.playAudio(plaintext)
    # [ Checkpoint  06]  Speaking  Question  : <QUESTION>
    print(time() + "Speaking  Question  :", plaintext)
    ...
    print(time() + "Connecting  to ", server_ip, "  on  port ", server_port)

    sending = encodeMessage(plaintext)
    socket.send(sending)
    # [ Checkpoint  08]  Sending  data : <PICKLED QUESTION PAYLOAD>
    print(time() + "Sending  data : ", sending)
    ...
    print(time() + "Decrypt :  Using Key : ", data[0], "|  Plaintext :", plaintext)

    watson.playAudio(plaintext)
    # [ Checkpoint  11]  Speaking Answer  : <ANSWER>
    print(time() + "Speaking Answer  : ", plaintext)
